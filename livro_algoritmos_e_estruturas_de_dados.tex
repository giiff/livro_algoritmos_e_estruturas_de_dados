...
As matrizes de adjacência para o grafo da Figura~\ref{Grafos01} estão abaixo representadas nas Matrizes~\ref{Grafos_matrizes_adjacencia}.
\begin{center}
\begin{figure}
\[
\left(
...
a & \textbf{0} & ~ & ~ & ~ & ~ \\
b & \textbf{1} & \textbf{0} & ~ & ~ & ~ \\
c & \textbf{1} & \textbf{1} & \textbf{0} & ~ & ~ \\
d & \textbf{0} & \textbf{1} & \textbf{1} & \textbf{0} & ~ \\
e & \textbf{0} & \textbf{0} & \textbf{1} & \textbf{1} & \textbf{0} \\
\end{array}\right)
\]
\caption{Matrizes de adjacência para grafo não direcionado (esquerda) e para grafo direcionado (direita).}
\label{Grafos_matrizes_adjacencia}
\end{figure}
\end{center}
...
Tomando o mesmo exemplo do grafo da Figura~\ref{Grafos01}, teríamos uma lista de adjacências como mostrada na Figura~\ref{Grafos_Lista_Adjacencia}.
...
\end{tikzpicture}
\caption{Exemplo de lista de adjacência.}
\label{Grafos_Lista_Adjacencia}
\end{figure}

\subsection{Grafos como objetos (Programação orientada a objetos)}

A programação oerientada a objetos permite-nos abstrair em modelos chamados classes, um novo ``tipo'' de dados que possa representar uma instância de um problema do ``mundo real''. Em se tratando de grafos, os vértices, arestas e o próprio grafo podem ser objetos dessas respectivas classes.
Uma implementação, dessa abordagem está disponível no~\href{https://github.com/giiff/livro_algoritmos_e_estruturas_de_dados/tree/master/Code/Python}{GitHub}, no mesmo repositório deste material.

\subsection{Busca em Grafos}
\indent

A busca em profundidade (\textit{Deep-First Search}) é um algoritmo para percorrer os vértices de um grafo.
Dado um grafo $G = (V,E)$ e um vértice inicial $v_1$, a estratégia da busca em profundidade é buscar, sempre que possível, o mais profundo no grafo.
Partindo de um vértice inicial $v_1$ a ideia é atravessar as arestas e outros vértices a ele conectados~\cite{ziviani2007}.
Quando todas as arestas adjacentes a $v_1$ tiverem sido exploradas, a busca anda para trás (\textit{backtrack}) para explorar outros vértices que saem do vértice $v_1$ que ainda não tenham sido visitados.
...
A Figura~\ref{Grafos_DFS} mostra um exemplo ilustrado do funcionamento do algoritmo de busca em largura.
\begin{figure}[htbp]
	\centering
	\includegraphics[width=\textwidth]{Pictures/DFS_Diagrama.png}
	\caption[Exemplo de busca em profundidade.]{Exemplo de busca em profundidade.}
	\label{Grafos_DFS}
\end{figure}
...
Dado um grafo $G = (V,E)$ e um vértice $v_1$, inicialmente inclui-se o vértice $v_1$ em uma lista de vértices visitados.
Busca-se então, todos os vértices adjacentes a $v_1$, os quais são incluídos em uma fila para ser visitados.
Os vértices vão sendo incluídos na lista de visitados à medida que vão sendo excluídos da fila de visitação.
O mesmo procedimento é executado sucessivamente até que todos os vértices tenham sido visitados e não haja mais nenhum na fila para visitação.
Tipicamente a complexidade de tempo desse algoritmo também é de $O(|V| + |E|)$ para percorrer o grafo inteiro~\cite{cormen2009}.
A Figura~\ref{Grafos_BFS} mostra um exemplo ilustrado do funcionamento do algoritmo de busca em largura.
\begin{figure}[htbp]
	\centering
	\includegraphics[width=\textwidth]{Pictures/BFS_Diagrama.png}
	\caption[Exemplo de busca em largura.]{Exemplo de busca em largura.}
	\label{Grafos_BFS}
\end{figure}

\subsection{Caminho de Euler}

Um caminho Euleriano ou caminho de Euler, é um caminho em um grafo $G = {V,E}$, direcionado ou não, em que cada aresta só pode ser visitada uma única vez, enquanto os vértices podem ser visitados mais de uma vez.
Se o caminho de Euler inicia e termina em um mesmo vértice, então esse caminho é chamado de circuito ou ciclo de Euler.
Um caminho de Euler ocorre nas seguintes circunstâncias:
\begin{itemize}
	\item Se todos os vértices do grafo $G$ tem um grau par.
	\item Se a quantidade de vértices com grau ímpar for igual a $2$.
\end{itemize}
O diagrama da Figura~\ref{Grafos_Euler} mostra um caminho de Euler para um grafo direcionado.
Nele, cada aresta foi visitada uma única vez. 
O caminho de Euler foi possível porque, neste caso, a quantidade de vértices com grau impar é igual a $2$, ou seja, vértices $A$ e $C$.
\begin{figure}[htbp]
	\centering
	\includegraphics[width=.5\textwidth]{Pictures/Euler_Path.png}
	\caption[Caminho de Euler.]{Exemplo de caminho de Euler em um grafo direcionado.}
	\label{Grafos_Euler}
\end{figure}

\subsection{Caminho Hamiltoniano}

Um caminho Hamiltoniano é um caminho em um grafo $G = {V,E}$ direcionado ou não, em que cada vértice é visitado apenas uma vez.
O problema de checar se um grafo contém um caminho Hamiltoniano é NP-completo, ou seja, é um problema da classe de problemas não-deternimísticos polinomiais (NP), cujas soluções não podem ser verificadas em tempo polinomial.
O diagrama da Figura~\ref{Grafos_Hamiltoniano} mostra um caminho Hamiltoniano para um grafo não direcionado.
\begin{figure}[htbp]
	\centering
	\includegraphics[width=.5\textwidth]{Pictures/Hamiltoniano_Path.png}
	\caption[Caminho Hamiltoniano.]{Exemplo de caminho Hamiltoniano em um grafo não direcionado.}
	\label{Grafos_Hamiltoniano}
\end{figure}
