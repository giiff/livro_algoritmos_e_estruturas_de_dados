...
Uma função é $O(g(n))$ se, para todos os valores de $n$ maiores que $n_0$, o valor da função $f(n)$ está abaixo de $g(n)$.
Geralmente não é uma tarefa trivial determinar se uma certa função $g(n)$ é $O(f(n))$.
Para ilustrar melhor o domínio assintótico de uma função, sejam as funções $g(n)=6n^2$ e $f(n) = 5n^2~+~15$, onde $n~\geq~4$. 
